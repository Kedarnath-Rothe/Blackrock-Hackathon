...
const AdminContacts = () => {
  ...
  const getContactsData = async () => {
    try {
      const response = await fetch("https://blackrock-hackathon.vercel.app/api/admin/contacts", {
        method: "GET",
        headers: {
  ...
  const deleteContactById = async (id) => {
    try {
      const response = await fetch(`https://blackrock-hackathon.vercel.app/api/admin/contacts/delete/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: authorizationToken,
        }
      ...
      <section className="admin-contacts-section" >
        ...
        <div className="container admin-users">
          {contactData.map((curElem, index) => {
            const { _id, username, email, message, aadhar_card } = curElem;
            ...
            return (
              <div key={index}>
                ...
                <p>{message}</p>
                {aadhar_card && (
                  <p className="aadhar" onClick={() => viewAadharImage(`${aadhar_card}`)} > 
                      View Aadhar 
                  </p>
                )}
